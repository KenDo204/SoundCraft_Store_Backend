...
export class CloudinaryService {
  ...
  async uploadImageBanners(file: Express.Multer.File): Promise<any> {
    return new Promise((resolve, reject) => {
      const upload = cloudinary.uploader.upload_stream(
        { folder: 'soundcraft/banners' }, // Tên folder bạn muốn lưu trên Cloudinary
        (error, result) => {
          if (error) return reject(error);
          resolve(result);
        },
  ...
  async uploadImageCategories(file: Express.Multer.File): Promise<any> {
    return new Promise((resolve, reject) => {
      const upload = cloudinary.uploader.upload_stream(
        { folder: 'soundcraft/categories' }, // Tên folder bạn muốn lưu trên Cloudinary
        (error, result) => {
          if (error) return reject(error);
          resolve(result);
        },
  ...
  async uploadImageProducts(file: Express.Multer.File): Promise<any> {
    return new Promise((resolve, reject) => {
      const upload = cloudinary.uploader.upload_stream(
        { folder: 'soundcraft/products' }, // Tên folder bạn muốn lưu trên Cloudinary
        (error, result) => {
          if (error) return reject(error);
          resolve(result);
        },
